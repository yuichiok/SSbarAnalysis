...
TH1F * Efficiency_Correction( TH1F * h, TString name, TFile * file )
{
  ...
  for (int ibin = 1; ibin < nbins + 1; ibin++){
    ...
    Float_t effdiv   = binc_h / binc_eff;
    Float_t effdiv_err = sqrt(effdiv);

    // cout << effdiv_err << endl;
    corrected->SetBinContent(ibin,effdiv);
    corrected->SetBinError(ibin,effdiv_err);

  }



  // Int_t genN = h_gen_N_K_cos->Integral();
