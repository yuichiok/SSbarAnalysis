...
void main_pq_BGFit( TFile *files[] )
{
  ...
  h_gen_uu_qcos_scale->Fit("f_gen_uu","MNRS");
  h_gen_dd_qcos_scale->Fit("f_gen_dd","MNRS");

  // extract the parameters from the uu/dd fits
  double gen_uu_par[2];
  double gen_dd_par[2];
  ...
  std::copy(gen_dd_par, gen_dd_par + 2, gen_ud_par + 2);
  f_reco_ud_mix->SetParameters(gen_ud_par);

  // Fit to the data
  h_reco_Pi_pq_cos->Fit("f_reco_ud_mix","MNRS");

  double reco_ud_par[4];
  f_reco_ud_mix->GetParameters(reco_ud_par);

  // Reco uu/dd functions
  TF1 * f_reco_uu      = new TF1("f_reco_uu","[0]*(1+x*x)+[1]*x",-0.9,0.9);
  TF1 * f_reco_dd      = new TF1("f_reco_dd","[0]*(1+x*x)+[1]*x",-0.9,0.9);
