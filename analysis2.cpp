void analysis2(char dz, string mode) {
    ...
    TH1F* h1 = (TH1F*)file->Get(Form("/Own/PS/%c0_sigma_%c0_P_%s",dz,dz,mode.c_str()));
    TH1F* h2 = (TH1F*)file->Get(Form("/Own/PS/%c0_sigma_%c0_S_%s",dz,dz,mode.c_str()));
        
    h1->Scale(1./h1->GetEntries());
    h1->SetTitle(Form("%c_{0} %s prong",dz,mode.c_str()));
    h1->GetXaxis()->SetTitle(Form("#frac{%c_{0}}{#sigma(%c_{0})}",dz,dz));
